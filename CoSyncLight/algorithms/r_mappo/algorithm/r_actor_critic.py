...
from algorithms.utils.act import ACTLayer, ACTLayer_TopK
from algorithms.utils.popart import PopArt
from utils.util import get_shape_from_obs_space
import torch.nn.functional as F

from DCM.dcm import DynaCo
from DCM.stsgcn_config import stsgcn_get_config
from algorithms.r_mappo.algorithm.trans_net import *
...
class R_Actor_Trans(nn.Module): 
    def __init__(self, args, obs_space, action_space, device=torch.device("cpu")):
        ...
        if args.use_trans_hidden:   
            ...
            self.hidden_size = 2 * 7 * 8 
        obs_shape = get_shape_from_obs_space(obs_space) 

        self.model = MLPBase(args, (obs_shape[0],), device=device)
       
        stsgcn_config = stsgcn_get_config
 
        argsDict = stsgcn_config.__dict__
        with open(str(self.args.run_dir) + '/args.txt', 'a') as f:
            f.writelines('\n------------------ STSGCN arguments------------------' + '\n')
            for eachArg, value in argsDict.items():
                f.writelines(eachArg + ' : ' + str(value) + '\n')   
        self.dynaco = DynaCo(stsgcn_config=vars(stsgcn_config), num_agents=self.num_agents, num_T=self.T, obs_shape=obs_shape[0], batch_size=1 , num_of_out_features=args.trans_hidden, device=self.device, args=args)
        ...
        if args.use_kl:
            ...
            self.score = ACTLayer_TopK(group_action_space, self.hidden_size, self._use_orthogonal, self._gain, args=args)
        
        self.countt = 0

    def forward(self, obs, historical_obs, cur_epi=None, available_actions=None, deterministic=False, use_eval=False):
        """
        ...
        historical_obs = check(historical_obs).to(**self.tpdv)

        if available_actions is not None:
            available_actions = check(available_actions).to(**self.tpdv)

  
        actor_features_ = self.model(obs)   

        agg_states, att_score, adj, adj_logits, (h_n, c_n) = self.dynaco(historical_obs, cur_epi, use_eval=use_eval) 


        adj_log_probs = F.log_softmax(adj_logits, dim=-1)  
 
        actor_features_new = torch.cat((actor_features_, agg_states), dim=-1)

        actor_features_new = actor_features_new.reshape(-1, actor_features_new.shape[-1])  
 
        actions, action_log_probs = self.act(actor_features_new, available_actions, deterministic)

 
        return actions, action_log_probs, actor_features_new, agg_states, adj, adj_log_probs, (h_n, c_n), att_score  

    def evaluate_actions(self, obs, agg_states, his_hn, his_cn, historical_obs, rnn_states, action, masks, available_actions=None, active_masks=None, score_batch=None, trans_masks=None):
        ...
        if len(actor_features_.shape) == 3: 
            score_batch = check(score_batch).to(**self.tpdv)
            his_hn = check(his_hn).to(**self.tpdv)
            his_cn = check(his_cn).to(**self.tpdv)
            ...
            agg_states_new, att_score, score_action_log_probs, score_dist_entropy, action_logits = self.dynaco.evaluate_actions(historical_obs, his_hn, his_cn)  
 
            actor_features_new = torch.cat((actor_features_, agg_states_new), dim=-1)
        ...
        return action_log_probs, dist_entropy, score_action_log_probs, score_dist_entropy, action_logits, att_score
    ...
    def forward(self, cent_obs, rnn_states, masks, topk_index=None, backward=False):
        ...
        if not backward:
            ...
            if cent_obs.type() == 'torch.cuda.FloatTensor':
                cent_obs = cent_obs.cpu().detach().numpy()  
...
class R_Critic(nn.Module):
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        if args.use_trans_hidden:
            self.base = base(args, (args.trans_hidden+args.trans_hidden,), device)
        else:
            trans_hidden = 2 * 7 * 8  
...
class R_Critic_Trans_all(nn.Module):
    ...
    def forward(self, obs, rnn_states, masks, topk_index, backward=False):
        ...
        for i_agent in range(self.args.num_agents):
            obs_i = obs[torch.arange(roll_outs).unsqueeze(1), topk_index[:, i_agent], :]  
            masks_i = masks[torch.arange(roll_outs).unsqueeze(1), topk_index[:, i_agent], :]   
            rnn_states_i = rnn_states[torch.arange(roll_outs).unsqueeze(1), topk_index[:, i_agent]]   
