...
class Intersection:
    ...
    def reset_tl_logic(self):
        ...
        for i in range(len(self.lans_link)):
            ...
            # if i not in np.concatenate(self.phase_index).astype(int):
        ...
        for i in range(len(self.entering_sequence_NWSE)):
            if self.entering_sequence_NWSE[i] != -1:
                ...
                for s in self.seq:
                    ...
                    if s in self.entering_approaches2phase[seq_i]:
                        ...
                        if "stop_car_num" in self.state:
                            obs['stop_car_num'] = stop_car_num / len(self.entering_approaches2phase[seq_i][s])
                        ...
                        if 'queue_length' in self.state:
                            obs['queue_length'] = queue_length / len(self.entering_approaches2phase[seq_i][s])
                ...
                for _ in self.seq:
                    mask.append(1)
                    main_direction_lanes.append({})
        return main_direction_lanes, mask, self.nearset_inter, self.location
