...
from envs.sumo_files_marl.env.intersection import Intersection
...
class TSCSimulator:
    def __init__(self, config, port, not_default=True, output_path=None):
        ...
        self.sample_interval = config.get("sample_interval")
        self.config = config
    ...
    def get_average_trip_time(self):
        for veh_id in self.sim.vehicle.getIDList():
            self.veh_start_time[veh_id] = self.sim.vehicle.getDeparture(veh_id)
            self.veh_end_time[veh_id] = self.sim.vehicle.getLastActionTime(veh_id)
        total_travel_time = 0
        for veh in self.veh_start_time.keys():
            total_travel_time += (self.veh_end_time[veh] - self.veh_start_time[veh])
        self.average_trip_time = total_travel_time / len(self.veh_start_time) if len(self.veh_start_time) != 0 else 0
    ...
    def _do_action(self, action):
        if self.action_type == 'select_phase':
            ...
            self.tmp_last_time = self._current_time
            self._current_time += self._yellow_duration
            
            self.tmp_last_time+=1
            self.sim.simulationStep(self.tmp_last_time)
            while self.tmp_last_time < self._current_time:
                self.tmp_last_time += self.sample_interval
                self.sim.simulationStep(self.tmp_last_time)
                self.get_average_trip_time()
                self.historical_obs.append(self._get_state())
            for tl, a in action.items():
            ...
            while self.tmp_last_time < self._current_time:
                ...
                if self.tmp_last_time + self.sample_interval > self._current_time:
                    self.tmp_last_time = self._current_time
                else:
                    self.tmp_last_time += self.sample_interval
                self.sim.simulationStep(self.tmp_last_time)
    ...
    def step(self, action):
        ...
        info['average_trip_time'] = self.average_trip_time
        info['historical_obs'] = self.historical_obs
        info['all_reward'] = self.all_reward
